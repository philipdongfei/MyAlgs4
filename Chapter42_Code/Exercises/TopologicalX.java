...
public class TopologicalX {
    ...
    /**
     * Determines whether the edge-weighted digraph has a
     ...
     * @return {@code true} if the digraph has a topological order (or equivalently, if the digraph is a DAG), and {@code false} otherwise
     */
    public boolean hasOrder(){
        return order != null;
    }
    /**
     * The the rank of vertex in the topological order;
     * -1 if the digraph is not a DAG
     *  @param v vertex
     *  @return the position of vertex in a topological order
     *  of the digraph; -1 if the digraph is not a DAG
     *  @throw IllegalArgumentException unless {@code 0 <= v < V}
     */
    ...
    private boolean check(EdgeWeightedDigraph G){
        ...
        if (hasOrder()) {
            ...
            // check that ranks provide a valid a topological order
            for (int v = 0; v < G.V(); v++) {
                for (DirectedEdge e : G.adj(v)){
                    int w = e.to();
                    if (rank(v) > rank(w)){
