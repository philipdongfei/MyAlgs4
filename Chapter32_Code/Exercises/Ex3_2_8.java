...
public class Ex3_2_8 {
    ...
    private static int optCompares(int n) {
        ...
        // Compute the compares in all levels, except the
        // last (because the last level may not be complete)
        for (int i = 1; i <= height; i++)
        {
            totalCompares += i * Math.pow(2, i-1);
            numberOfNodesBeforeLastLevel += Math.pow(2, i-1);
        }
        // Add compares required to reach the nodes in the
        // last level.
        ...
        int numberOfComparesRequiredByRandomSearchHit = 
            (totalCompares/n) + 1;
