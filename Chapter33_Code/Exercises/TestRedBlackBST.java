...
public class TestRedBlackBST{
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("After deleting the smallest " + st.size()/2 + 
                " keys");
        StdOut.println("----------------------");
        for (String s : st.keys())
            StdOut.println(s + " " + st.get(s));
        ...
        RedBlackBST<Integer, Integer> st2 = new RedBlackBST<Integer,
            Integer>();
        for (int i = 0; i < n; i++) {
            st2.put(i, i);
            int h = st2.height();
            StdOut.println("i = " + i + ", height = " + h +
                    ", size = " + st2.size());
        }
