...
public class MultiST<Key extends Comparable<Key>, Value> {
    private class Node {
        private Key key;
        private Value val;
    ...
    public Iterable<Value> getAll(Key key)
    {
        ...
        return getAll(root, queue, key);
    }
    private Iterable<Value> getAll(Node n,Queue<Value> qu,  Key key){
        if (n == null)
            return qu;
        int compare = key.compareTo(n.key);
        if (compare < 0)
            return getAll(n.left, qu, key);
        else if (compare > 0)
            return getAll(n.right, qu, key);
        else
        {
            qu.enqueue(n.val);
            return getAll(n.left, qu, key);
